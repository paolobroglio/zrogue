...
const PositionComponent = struct { x: f32, y: f32 };

const KeyboardControlledComponent = struct { up_velocity: f32, down_velocity: f32, right_velocity: f32, left_velocity: f32 };

const SpriteComponent = struct { texture_id: []const u8, width: i32, height: i32, src_rect: rl.Rectangle };
...
pub fn ComponentStore(comptime T: type) type {
    return struct {
        ...
        pub fn init(allocator: std.mem.Allocator) Self {
            return Self{ .components = std.ArrayList(T).init(allocator), .entity_to_index = std.AutoHashMap(Entity, usize).init(allocator), .index_to_entity = std.ArrayList(Entity).init(allocator) };
        }
